...
//			System.out.println("non-or card");
		...
		for(Resource r: cardGoods){
			...
			if( orOpts != null){				
				...
				orList.add(srl);
				if(c.getColor() == CardColor.BROWN || c.getColor() == CardColor.GREY)
					sellOrList.add(srl);
...
//				System.out.println("\n\nbuilding:  " +  card);
//				System.out.println("building:total:  " +  cmdOpt.getTotalNeeded());
//				System.out.println("building:needs:  " +  cmdOpt.getNeedsSRL());
//				System.out.println("\nbuilding:leftRes:  " +  leftSRL );
//				System.out.println("building:rightRes:  " +  rightSRL );				
//				System.out.println("needsBuyDecision: " + cmdOpt.needsBuyDecision() );
				...
				if( ! cmdOpt.needsBuyDecision() ){
					...
					player.makeBuyDecision(cmdOpt.getNeedsSRL(), leftSRL, rightSRL );
						// left first
...
//					System.out.println(card + " is instead sold for 3 coins ");
					GameManager.getManager().out[getIndex()].append("Selling " + card);
					this.addToCoins(3);
					discardCard(card);
					break;
				}
				...
				if(lCost > 0){
					...
					System.out.print("Paying " + this.leftNeighbor.getName() + " " + lCost);
					GameManager.getManager().out[getIndex()].append("Paying " + lCost + " to " + leftNeighbor.getName() + "\n");
					GameManager.getManager().out[leftNeighbor.getIndex()].append("\nPaid to us " + lCost+ " by " + getName());
				...
				if(rCost > 0){
					...
					System.out.print("Paying " + this.rightNeighbor.getName() + " " + rCost);
					GameManager.getManager().out[getIndex()].append("Paying " + rCost + " to " + rightNeighbor.getName() + "\n");
					GameManager.getManager().out[rightNeighbor.getIndex()].append("\nPaid to us " + rCost+ " by " + getName());
...
//			command = (this.canBuildStage() )? cmd : Command.NONE;
		...
		case NONE:
		default:
			System.out.println("OOOPs, I haven't implemented that ability into takeTurn yet");				
		}
	...
	public void takeTurn( Hand hand, int trnNum ){
		...
		if(trnNum == 6 && saveSeventhCard ){
			System.out.println("\nTESTING: " + this.toString() + " has: " + hand.size() + " cards left in after 6th turn ");
	...
	// TODO need to add logic for using 'or' x3 and x4 resources and then neighbor.
	/**
		...
		for(int i = 0; i < hand.size(); i++ ){
			...
			for(Card crd : structures){
				if(crd.getName().equals(card.getName()) ){
					options[i] = new CommandOption(card, 0, null, null, null, false, false);
					options[i].setReason(" already has one ");
//					System.out.println("board already has a copy of " + card.getName());
//					GameManager.getManager().out[getIndex()].append("\nBoard already has a copy of " + card.getName());
//					GameManager.getManager().out[0].append("\nSIDE NOTE:" + this.getName()+ " already has a copy of " + card.getName());					
					continue loop;
				}
			}
			...
			if( freeList.contains( card.getName())){						
				...
				options[i] = new CommandOption(card, 0, null, null, null, true, true);
				...
				if( canBuild(card, cardCost) ){
					...
					if(getTotalCoins() == 0){  
						options[i] = new CommandOption(card, totalCost, null, null, null, false, false);
						continue;
					}
					// not enough money to cover all the needed goods
					if(cardCost.getNumRawGoods() * (leftRawCost < rightRawCost ? leftRawCost : rightRawCost) +
					   cardCost.getNumManfGoods()* this.manfCost > this.getTotalCoins()	) {
						options[i] = new CommandOption(card, totalCost, null, null, null, false, false);
						continue;
					}
									
					int rightProvide = rightNeighbor.canBuy(new SimpleResList(cardCost) );
					int leftProvide = leftNeighbor.canBuy(new SimpleResList(cardCost) ) ;
					System.out.println("East Neighbor could provide " +  rightProvide);
					System.out.println("West Neighbor could provide " +  leftProvide);
					...
					if(testCost1.getTotalRes() == 0){
						...
						SimpleResList rightProv = SimpleResList.subtract(cardCost, testCost1);
						SimpleResList leftProv = SimpleResList.subtract(cardCost, testCost2);
						
						options[i] = new CommandOption(card, totalCost, cardCost, rightProv, leftProv, true, false);
					}
					else
						options[i] = new CommandOption(card, totalCost, null, null, null, false, false);
				}						
...
//			System.out.println("\t\tcanBuild: card is free" );
		...
		ArrayList<SimpleResList> compOrList = new ArrayList<SimpleResList>( orList );
		...
		while(listChanged){
			...
			if(cardCost.getTotalRes() == 0 || cardCost.getTotalRes() <= compOrList.size() ){				
				return true;
			}
		}
		System.out.println("canBuild:::numOrsLeft " + compOrList.size() + " goodStillneeded: " + cardCost.getTotalRes());
	...
	public boolean canBuildNextStage( ){
		System.out.print("Checking if " + stages[stagesCompleted] + " can be built:");
		SimpleResList costSRL = SimpleResList.buildCostList( stages[stagesCompleted] );
		System.out.println( (canBuildStage(stages[stagesCompleted], costSRL ) ? " true ": " false ") );	
		// TODO  needs code to see if there is enough money to buy the goods from the neighbors(if they have the goods).....look buildCommandOptions
		return canBuildStage(stages[stagesCompleted], costSRL );  // this is only checking to see if this board can afford it
	}
	...
	public boolean canBuildStage(Stage bld, SimpleResList stageCost){
		...
		while(listChanged){
			...
			if(stageCost.getTotalRes() == 0 || stageCost.getTotalRes() <= compOrList.size() ){				
				return true;
			}
		}
		System.out.println("canBuildStage:::numOrsLeft " + compOrList.size() + " goodStillneeded: " + stageCost.getTotalRes());
		if(! canAfford ){
			
			System.out.println("\t\t Could buy " + this.leftNeighbor.canBuy( new SimpleResList(stageCost)) + " resources from left neighbor" );
			System.out.println("\t\t Could buy " + this.rightNeighbor.canBuy( new SimpleResList(stageCost)) + " resources from right neighbor" );
		}
		return canAfford;
	}
	...
	public int canBuy(SimpleResList cardCost){
		...
		ArrayList<SimpleResList> copySellOrList = new ArrayList<SimpleResList>( sellOrList );
...
//					int dif = cardCost.getTotalRes();
			...
			if(cardCost.getTotalRes() == 0 || cardCost.getTotalRes() <= copySellOrList.size() ){				
				return totalNeeded;
			}
		}		
...
/*
	...
	@SuppressWarnings("unchecked")	// this one uses the old complicated way with the EnumMaps
	public boolean canBuildOldWay(Card bld){
		Set<Resource> costSet = null;
		EnumMap<Resource,Integer> resNeededMap = new EnumMap<Resource,Integer>(Resource.class) ;
		costSet = bld.getCosts();
		boolean canAfford = true;
		int numResNeeded, resProvided;
		for(Resource res: costSet ){   			// calculate resources needed	
			if( res == Resource.FREE ){
//				System.out.println("\tis free to build " );
				return true;						
			}
			numResNeeded = bld.costCnt(res);
			resProvided = ( goods.containsKey(res) ? goods.get(res) : 0 );
			if( numResNeeded > resProvided ){
				int buyNum =  numResNeeded - resProvided;
				canAfford = false;
//				System.out.println("\tnot enough " + res.toString() + ": needs " + buyNum + " more");
				resNeededMap.put(res, buyNum);
			}									
		}
	...
	//######################################
	ArrayList<Card> copyOrCards = ( ArrayList<Card> ) orCards.clone();
	boolean resNeeded;
	int nrNeeded;
//	System.out.println("copyOrCards size before: " + copyOrCards.size());
	ArrayList<Card> removeList = new ArrayList<Card>();
	Resource[] resArray;
	for(Card c : copyOrCards){
//		System.out.println(c + " is in copyOrCards");
		resNeeded = false;
		nrNeeded = 0;
		for(Resource r : c.getGoods() ){
			resArray = r.getOptionalRes();
			for (Resource res : resArray ){
//				System.out.println(res + " checking if needed ");
				if(resNeededMap.containsKey(res)){
					resNeeded = true;
					nrNeeded++;
//					System.out.println(c + " is needed ");					
				}
			}
			if( resNeeded == false)
				removeList.add(c);
		}
		// use card to reduce resources needed
		for(Resource r : c.getGoods() ){
			resArray = r.getOptionalRes();
			for (Resource res : resArray ){
//				System.out.println(res + " checking if needed ");
				if(resNeededMap.containsKey(res)){
					resNeededMap.put(res, resNeededMap.get(res)-1);
					if (resNeededMap.get(res) == 0)
						resNeededMap.remove(res);						
					removeList.add(c);	
					System.out.println(c + " is being used for " + res);					
				}
			}
	...
	if( ! removeList.isEmpty() )
		for( Card c : removeList)
			copyOrCards.remove(c);
	
	
//	System.out.println("copyOrCards size after: " + copyOrCards.size());
	
	
	if (resNeededMap.isEmpty())
		canAfford = true;
	else
		for(Resource r : resNeededMap.keySet()){
			System.out.println(r + " still needs: " + resNeededMap.get(r));
		}	
		return canAfford;
	}
