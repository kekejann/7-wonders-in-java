...
public class HumanDialogBasedPlayer implements Player {
	...
	/** The commands. */
	private Command[] commands = {Command.BUILD_CARD, Command.SELL_CARD, Command.BUILD_STAGE};
	
	/* (non-Javadoc)
	 * @see org.tjuscs.sevenwonders.core.Player#setBoard(org.tjuscs.sevenwonders.core.Board)
	 */
	...
	/* (non-Javadoc)
	 * @see org.tjuscs.sevenwonders.core.Player#makeChoice(org.tjuscs.sevenwonders.core.CommandOption[])
	 */
	public CommandOption makeChoice(CommandOption[] options) {
        ...
        nPanel.add(new JLabel( "  Can build next Stage: " + board.canBuildNextStage() + "   " + board.stages[board.stagesCompleted].cost));
...
//        radioButtons[0].setSelected(true);
       ...
       // cmdRadioButtons[0].setSelected(true);
		...
		JOptionPane.showMessageDialog(null, bigPanel,  board.getName(),   JOptionPane.PLAIN_MESSAGE);
		
		options[choice].setCommand( cmdChoice );
		System.out.println("HumanDialogPlayer Chose: " + options[choice].toString());
		return options[choice];
	}

	/* (non-Javadoc)
	 * @see org.tjuscs.sevenwonders.core.Player#makeBuyDecision(org.tjuscs.sevenwonders.core.SimpleResList, org.tjuscs.sevenwonders.core.SimpleResList, org.tjuscs.sevenwonders.core.SimpleResList)
	 */
	@Override
	...
	class CardOptionListener implements ActionListener{	
		...
		public void actionPerformed(ActionEvent e) {
		    ...
		    System.out.println("HumanDialogBasedPlayer:: choice changed to " + choice);
		}
	}
	...
	class CommandOptionListener implements ActionListener{	
		...
		public void actionPerformed(ActionEvent e) {
		    ...
		    System.out.println("HumanDialogBasedPlayer:: command changed to " + cmdChoice);
		}
	}
	...
	class BoardPanel extends JPanel {				
		...
		public void update(){
			int num;
			if(  board.goods.containsKey(Resource.BRICK) && (num = board.goods.get(Resource.BRICK) ) > 0 )
				labels[0].setText("Brick: " + num);
			if( board.goods.containsKey(Resource.ORE) && ( num = board.goods.get(Resource.ORE) ) > 0 )
				labels[1].setText("Ore: " + num);
			if( board.goods.containsKey(Resource.STONE) && ( num = board.goods.get(Resource.STONE) ) > 0 )
				labels[2].setText("Stone: " + num);
			if( board.goods.containsKey(Resource.WOOD) && ( num = board.goods.get(Resource.WOOD) ) > 0 )
				labels[3].setText("Wood: " + num);
			if( board.goods.containsKey(Resource.CLOTH) && ( num = board.goods.get(Resource.CLOTH) ) > 0 )
				labels[4].setText("Cloth: " + num);
			if( board.goods.containsKey(Resource.GLASS) && ( num = board.goods.get(Resource.GLASS) ) > 0 )
				labels[5].setText("Glass: " + num);
			if( board.goods.containsKey(Resource.PAPYRUS) && ( num = board.goods.get(Resource.PAPYRUS) ) > 0 )
				labels[6].setText("Papyrus: " + num);
			
			if( board.goods.containsKey(Resource.WOOD_BRICK) && ( num = board.goods.get(Resource.WOOD_BRICK) ) > 0 )
				labels[7].setText("wood/brick: " + num);
			if( board.goods.containsKey(Resource.BRICK_ORE) && ( num = board.goods.get(Resource.BRICK_ORE) ) > 0 )
				labels[8].setText("brick/ore: " + num);
			if( board.goods.containsKey(Resource.STONE_BRICK) && ( num = board.goods.get(Resource.STONE_BRICK) ) > 0 )
				labels[9].setText("stone/brick: " + num);
			if( board.goods.containsKey(Resource.STONE_WOOD) && ( num = board.goods.get(Resource.STONE_WOOD) ) > 0 )
				labels[10].setText("stone/wood: " + num);
			if( board.goods.containsKey(Resource.WOOD_ORE) && ( num = board.goods.get(Resource.WOOD_ORE) ) > 0 )
				labels[11].setText("wood/ore: " + num);
			if( board.goods.containsKey(Resource.ORE_STONE) && ( num = board.goods.get(Resource.ORE_STONE) ) > 0 )
				labels[12].setText("ore/stone: " + num);
			
			if( board.goods.containsKey(Resource.PAPYRUS_GLASS_CLOTH) && ( num = board.goods.get(Resource.PAPYRUS_GLASS_CLOTH) ) > 0 )
				labels[13].setText("papyrus/glass/cloth: " + num);
			if( board.goods.containsKey(Resource.BRICK_STONE_ORE_WOOD) && ( num = board.goods.get(Resource.BRICK_STONE_ORE_WOOD) ) > 0 )
				labels[14].setText("brick/stone/ore/wood: " + num);
						
			
			if( board.goods.containsKey(Resource.COG) && ( num = board.goods.get(Resource.COG) ) > 0 )
				labels[15].setText("SCI:Cog: " + num);
			if( board.goods.containsKey(Resource.COMPASS) && ( num = board.goods.get(Resource.COMPASS) ) > 0 )
				labels[16].setText("SCI:Compass: " + num);
			if( board.goods.containsKey(Resource.TABLET) && ( num = board.goods.get(Resource.TABLET) ) > 0 )
