...
public class SimpleResList {
	...
	public static SimpleResList buildCostList(Buildable thing){
		...
		temp.srl[0] = temp.srl[1] + temp.srl[2] + temp.srl[3] + temp.srl[4] + temp.srl[5] + temp.srl[6] + temp.srl [7];
		return temp;
	...
	public static Resource resourceAt(int index){
		...
		case 3: return Resource.STONE;
		case 4: return Resource.WOOD;
		case 5: return Resource.CLOTH;
		case 6: return Resource.GLASS;
		case 7: return Resource.PAPYRUS;
		default:
	...
	public String toString(){
		...
		if( srl[1] > 0 ) sb.append(" " + srl[1] + " brick ");
		if( srl[2] > 0 ) sb.append(" " + srl[2] + " ore ");
		if( srl[3] > 0 ) sb.append(" " + srl[3] + " stone ");
		if( srl[4] > 0 ) sb.append(" " + srl[4] + " wood ");
		if( srl[5] > 0 ) sb.append(" " + srl[5] + " cloth ");
		if( srl[6] > 0 ) sb.append(" " + srl[6] + " glass ");
		if( srl[7] > 0 ) sb.append(" " + srl[7] + " papyrus ");
		return sb.toString();
	}
	 ...
	 * @return the simple res list
	 */
	public static SimpleResList subtract(SimpleResList resList1, SimpleResList resList2 ){
		...
		newList.srl[0] = newList.srl[1] + newList.srl[2] + newList.srl[3] + newList.srl[4] + 
						 newList.srl[5] + newList.srl[6] + newList.srl [7];
		return	newList;
	}
	 ...
	 * @return the simple res list
	 */
	public static SimpleResList add(SimpleResList resList1, SimpleResList resList2 ){
		SimpleResList newList = new SimpleResList(resList1);
		for( int i = 1; i < newList.srl.length; i++ ){
			newList.srl[i] += resList2.srl[i];						
		}
